...
from app.rag.pinyin_converter import PinyinConverter, build_pinyin_index
...
class TermLibrary:
    def __init__(self):
        self.terms = ALGORITHM_TERMS.copy()
        self.asr_mapping = ASR_ERROR_MAPPING.copy()
    ...
    def export_to_json(self) -> str:
        return json.dumps({
            ...
            "pinyin_index": {k: {kk: vv for kk, vv in v.items() if kk != 'info'}
                            for k, v in self.pinyin_index.items()}
        }, ensure_ascii=False, indent=2)


_default_library: Optional[TermLibrary] = None


def get_default_library() -> TermLibrary:
    global _default_library
    if _default_library is None:
        _default_library = TermLibrary()
    return _default_library


def get_term_library() -> TermLibrary:
